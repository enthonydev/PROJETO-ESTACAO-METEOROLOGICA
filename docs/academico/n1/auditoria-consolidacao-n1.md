...
| Arquitetura e dados | Aprovado com pendências | O fluxo, o DER e a migration estão descritos; MQTT, PostgreSQL runtime e dashboard permanecem pendentes. |
...
| Afirmações técnicas sem evidência | Aprovado com ressalvas | Hardware, calibração, MQTT real, banco runtime, dashboard e campo estão marcados como projetados, pendentes ou bloqueados. |
...
O modelo de dados e a migration existem, mas não há execução contra PostgreSQL nesta etapa. O relatório usa “implementado como modelo” e não “persistência validada”.
